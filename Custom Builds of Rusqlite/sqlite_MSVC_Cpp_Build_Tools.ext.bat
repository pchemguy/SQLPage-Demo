...
if /I "%~1"=="sqlite3.exe" (set BUILD_EXE=1)
if /I "%~1"=="shell" (set BUILD_EXE=1)
if /I "%~1"=="exe" (set BUILD_EXE=1)
if "%BUILD_EXE%"=="1" (
    set TARGETS=sqlite3.exe
)
if not "%TARGETS:sqlite3.exe=%"=="%TARGETS%" (
    set WITH_EXTRA_EXT=0
)
...
:DOWNLOAD_ZLIB
set DISTRO=zlib.zip
set URL=https://zlib.net/zlib131.zip
...
if not exist "%BINDIR%\src" mkdir "%BINDIR%\src"

cd /d "%BUILDDIR%"
copy /Y sqlite3.c "%BINDIR%\src"
copy /Y sqlite3.h "%BINDIR%\src"
if exist fileio.c (copy /Y fileio.c "%BINDIR%\src")
if exist test_windirent.h (copy /Y test_windirent.h "%BINDIR%\src")

if exist sqlite3.dll (copy /Y sqlite3.dll "%BINDIR%")
if exist sqlite3.exe (copy /Y sqlite3.exe "%BINDIR%")
if exist sqlite3.def (copy /Y sqlite3.def "%BINDIR%")
if exist sqlite3.lib (copy /Y sqlite3.lib "%BINDIR%")
...
echo ^|^|   sqlite_MSVC_Cpp_Build_Tools.ext.bat sqlite3.exe OR shell OR exe          ^|^|
echo ^|^|                                                                            ^|^|
